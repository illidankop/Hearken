...
class SRP(DOA):
    ...
    def _process(self, X):
        ...
        def process(i):
            ...
            mode_vec = self.mode_vec[self.freq_bins, :, i]
            ...
            # Finally normalize
            return  sum_val / self.num_snap / self.num_freq / self.num_pairs

        # Erez
        @jit(nopython=True)
        def process_fast(mode_vec, n_points):
            # mode_mat = mode_vec[:, :, None]
            srp_cost = np.zeros(n_points)
            # mode_vec_s = mode_vec[:, :, 1]
            for n in range(n_points):
                ...
                # get the mode vector axis: (frequency, microphones)
                mode_vec_s = mode_vec[:, :, n]

                mode_mat = np.expand_dims(mode_vec_s, axis=2)
                mode_conj = np.conjugate(mode_mat)
                # mode_mat = mode_mat * mode_vec[:, None, :]
                mode_mat = mode_conj * np.expand_dims(mode_vec_s, axis=1)
        ...
        # srp_cost = Parallel(n_jobs=4, verbose=10,pre_dispatch='all', batch_size=8192)(delayed(process)(i) for i in range(self.grid.n_points))
        # start = time.time()
        # # for i in range(10):
        # srp_cost = process_fast(np.array(self.mode_vec[self.freq_bins, :, :]), self.grid.n_points) / self.num_snap / self.num_freq / self.num_pairs
        # end = time.time()

        # print(f'avg processing time:{(end - start)}')
        ...
        # end2 = time.time()
        # print(f'processing time2:{end2 - end}')
