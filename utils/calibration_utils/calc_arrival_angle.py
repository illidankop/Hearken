...
def estimate_coordinates(r,az,resp,az_division_factor, azspan):
    ...
    try:
        ...
        print(f'estimate_coordinates -following exception was cought {ex}')
...
def estimate_coordinates_toa(az,resp,az_division_factor, azspan):
    ...
    try:
        ...
        az_estimate = math.nan
        print(f'estimate_coordinates_toa -following exception was cought {ex}')
    return az_estimate
...
def calculate_aoa(sig, fs, c, element_coord, azspan, num_az,az_division_factor, elevation_angle, filter_response=None):
    """
    ...
    arrival_angle, cresp = generate_beams_and_estimate_coordinates(sig[:, :],azvec_model,fs,elref,r0_ref,element_coord,c,az_division_factor,azspan+20, filter_response)

    return arrival_angle, elevation_angle
    

def calculate_aoa_toa(sig, fs, c, element_coord, azspan, num_az, az_division_factor, elevation_angle):
    ...
    arrival_angle_toa = generate_toa_and_estimate_coordinates(sig[:, :], azvec_model, fs, elref, r0_ref,element_coord, c, az_division_factor, azspan+20)
...
def calc_arrival_angle(samples_of2sec, all_peaks_indexes, is_printed_sensor = False, is_calc_aoa_toa = False, window_sec=0.02, rate=32000):    
    """
    ...
    if is_printed_sensor:                                    
        dist_2_3 = 0.136
        dist_1_3 = 0.136        
        mic_loc = np.array([[0, 0, 0], [-dist_2_3, dist_1_3, 0], [-dist_2_3, -dist_1_3, 0]])
    else:                
        dist_2_3 = 0.11635
        dist_1_3 = 0.11635
        mic_loc = np.array([[0, 0, 0], [-dist_2_3, dist_1_3, 0], [-dist_2_3, -dist_1_3, 0]])            
    
    for event_time in all_peaks_indexes:
        arrival_angle = math.nan
        elevation_angle = math.nan

        signal = samples_of2sec[: , max(0, int((event_time - window_sec * 0.2) * rate)) : 
                                min(int((event_time + window_sec * 0.8) * rate), 2 * rate)]
        ...
        az_span = 180       #deg. Az span
        num_az = 50         # Number of azimuths for calculation over the span
        az_division_factor = 50  # Division ratio of the coarse step        
        filter_response, _ = generate_band_pass_filter(rate, data.T, lowest_freq_th)
        if is_calc_aoa_toa:
            arrival_angle, _ = calculate_aoa_toa(data, rate, speed_of_sound, mic_loc, az_span, num_az, az_division_factor, elevation_angle)            
        else:
            arrival_angle, _ = calculate_aoa(data, rate, speed_of_sound, mic_loc, az_span, num_az, az_division_factor, elevation_angle, filter_response)                            

        if math.nan(arrival_angle):
            print(f'Got invald calculated arriving angle!')
            continue
            
        if arrival_angle < 0:
            print(f'Fix negative arrival_angle: {arrival_angle}')
            arrival_angle += 360
        
        arrival_angles.append(arrival_angle)
        
    if len(arrival_angles) > 0:
        return sum(arrival_angles)/len(arrival_angles)
    else:      
        return -36000
