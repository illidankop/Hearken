...
class MobileNetV2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.spec_augmenter = SpecAugmentation(time_drop_width=8, time_stripes_num=1,
            freq_drop_width=1, freq_stripes_num=2)
    ...
    def forward(self, input, mixup_lambda=None):
        """
        Input: (batch_size, data_length)"""
        # print('input.shape=',input.shape)  # Debugging: Check the shape of the input
...
class Transfer_Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        ...
        # self.base.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2,
...
class Transfer_Cnn14_mel128(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num, freeze_base=True):
        """Classifier for a new task using pretrained Cnn14 as a sub module.
        """
        super(Transfer_Cnn14_mel128, self).__init__()
        audioset_classes_num = 527

        self.base = Cnn14_mel128(sample_rate, window_size, hop_size, mel_bins, fmin,
                          fmax, audioset_classes_num)
...
class Regression_MobileNetV2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num, freeze_base=False):
        """Classifier for a new task using pretrained Cnn14 as a sub module.
        """
        super(Regression_MobileNetV2, self).__init__()
        audioset_classes_num = 527

        self.base = MobileNetV2(sample_rate, window_size, hop_size, mel_bins, fmin,
                          fmax, audioset_classes_num)

        # Transfer to another task layer - regression
        self.fc_transfer = nn.Linear(1024, 1, bias=True)
...
class Combined_MobileNetV2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num, freeze_base=False):
        """Classifier for a new task using pretrained Cnn14 as a sub module.
        """
        super(Combined_MobileNetV2, self).__init__()
        audioset_classes_num = 527

        self.base = MobileNetV2(sample_rate, window_size, hop_size, mel_bins, fmin,
                          fmax, audioset_classes_num)

        # Transfer to another task layer - regression & classification
        self.fc_transfer = nn.Linear(1024, 2, bias=True)
